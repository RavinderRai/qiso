...
The following lemma shows that a protocol that has an average output density matrix which is close to a desired state also has, on average, an output density matrix which is close to the desired state on individual runs of the protocol.
...
\end{center}
Then by solving $1 - C \delta \leq p_S + (1 - p_S)(1 - C \delta)$ you get the desired lower bound: $p_S \geq \frac{C-1}{C}$.
\end{proof}

Now we want to consider the diamond norm distance between channels, and can give bounds in both directions between the trace distance and the fidelity $\sqrt{\braket{\psi | \rho | \psi}}$ using the Fuchs-van de Graaf inequalities:
\begin{equation}
...
From the second of these inequalities and from Lemma \ref{lemma5.4}, the following is implied:
